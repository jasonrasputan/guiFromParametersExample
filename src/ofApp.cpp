        ...
        if ( note != -1 )
        {
            if ( bNoteAdded == false )
            {
                ...
                */
                ...
                dt.value = note ;
                notes[arrayLocation].push_back( dt ) ;
                currentNote[arrayLocation] = notes[arrayLocation].size() - 1 ;
...
void ofApp::draw(){
    ...
    if ( notes[ 0 ].size() > 0 )
    {
        float w = ofGetWidth() / notes[ 0 ].size() ;
        if ( w > maxWidth )
            w = maxWidth ;
        ...
        {
            if ( i == (currentNote[0] + 1) )
                ofSetColor( 0 , 255 , 0 ) ;
            else
                ofSetColor( 255 ) ;
            int valueY = ofMap( notes[0][i].value , 0.0f , 5.0f , 0 , ofGetHeight() ) ;
